...
parser.add_argument("--dtc", type=int, choices=[0, 1, 2, 3], default=0, help="Number of DTCs")
...
def build_response(pid, value):
    ...
    if pid == PID_RPM:
        ...
        payload = [int(value)]
    elif pid == PID_NUM_DTC:
        # Bit 7: MIL status (1 = ON), bits 0–6: number of stored DTCs
        mil = 0  # or 1 if you want MIL ON
        payload = [(mil << 7) | (int(value) & 0x7F)]
...
try:
    ...
    print(f"Test 23: Starting at {start_str}. Duration: 12 minutes.")

    cycle = 0
    throttle_pos = min(100, cycle * 0.05)
    intake_temp = min(50, 20 + cycle * 0.01)
    num_dtc = args.dtc
    ...
    while True:
        ...
        if elapsed_time >= max_runtime_sec:
            ...
            print(f"Test 23 completed at {end_str}.")
            break
    ...
    print("Test 23 Emulator stopped manually.")
finally:
    bus.shutdown()
